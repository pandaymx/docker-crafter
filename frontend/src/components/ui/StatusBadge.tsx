...
export interface StatusBadgeProps extends React.HTMLAttributes<HTMLDivElement> {
  status: "running" | "stopped" | "exited" | "error" | "unknown" | "running-all" | "running-partial" | "stopped-all";
  showDot?: boolean;
}

export const StatusBadge: React.FC<StatusBadgeProps> = ({ status, className, children, showDot = true, ...props }) => {
  ...
  return (
    <div className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-mono border", {
      ...
      {showDot && (
        <div className="relative flex h-2 w-2 shrink-0 items-center justify-center">
          {(isRunning || isPartial) && (
            <span className={cn("absolute inline-flex h-full w-full rounded-full opacity-40 animate-pulse", {
              "bg-emerald-500": isRunning, "bg-amber-500": isPartial,
            })} />
          )}
          <span className={cn("relative inline-flex rounded-full h-1.5 w-1.5", {
            ...
            "bg-rose-500 h-2 w-2": status === "stopped" || status === "exited",
